...
//				returnEventError(event, "WS_RES_EXTENSION_STATE_FAIL");
		...
		case UC_REPORT_EXT_STATE:
			...
			switch (event.direct) {
				...
				// 12
				case UC_DIRECT_INCOMING:
					console.log("<<<===========================*** UC_DIRECT_INCOMING:"+ event.status);
					switch (event.status) {
						// 111
						case UC_CALL_STATE_IDLE:
							// 이전상태 비교
						...
						//case UC_CALL_STATE_RINGBACK:
							...
							console.log("%%%%%%%%%%%%%%%% myExtension----->"+ myExtension);
							if (myExtension == event.extension) {
								currentCallInfo = event;
								console.log("%%%%%%%%%%%%%%%% extension----->"+ currentCallInfo.extension);
								console.log("%%%%%%%%%%%%%%%% direct----->"+ currentCallInfo.direct);
								console.log("%%%%%%%%%%%%%%%% caller----->"+ currentCallInfo.caller);
								console.log("%%%%%%%%%%%%%%%% callee----->"+ currentCallInfo.callee);
							}
				...
				case UC_DIRECT_OUTGOING:
					...
					// 112	
					case UC_CALL_STATE_INVITING:
						console.log("<<<===========================*** UC_CALL_STATE_INVITING:"+ event.status);
